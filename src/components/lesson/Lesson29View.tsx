...
import { useLessonStore, type BlockId, type PracticeAnswer } from '@/lib/store'
import { LESSON_META } from '@/lib/lesson-data'
import Block4Udareniya from '@/components/lesson/Block4Udareniya'
import Block5Paronimy from '@/components/lesson/Block5Paronimy'
import Block6LexPravka from '@/components/lesson/Block6LexPravka'
import Block123Microtext from '@/components/lesson/Block123Microtext'
import Block22IVS from '@/components/lesson/Block22IVS'
import Block26SredstvaSvyazi from '@/components/lesson/Block26SredstvaSvyazi'
...
const BLOCKS: {
  ...
  },
  {
    ...
    color: 'teal',
    component: <Block5Paronimy />,
  },
  {
    id: 'block6',
    label: '№6. Лексическая правка',
    shortLabel: 'Лекс. правка',
    icon: <PenTool className="h-4 w-4" />,
    color: 'orange',
    component: <Block6LexPravka />,
  },
  {
    id: 'block123',
    label: '№1–3. Микротекст',
    shortLabel: 'Микротекст',
    icon: <BookOpen className="h-4 w-4" />,
    color: 'sky',
    component: <Block123Microtext />,
  },
  {
    id: 'block22',
    label: '№22. ИВС',
    ...
    color: 'violet',
    component: <Block22IVS />,
  },
  {
    id: 'block26',
    label: '№26. Средства связи',
    shortLabel: 'Ср-ва связи',
    icon: <Link2 className="h-4 w-4" />,
    color: 'amber',
    component: <Block26SredstvaSvyazi />,
  },
  {
    id: 'homework',
...
const colorClasses: Record<string, { bg: string; text: string; badge: string }> = {
  ...
  violet: { bg: 'bg-violet-100', text: 'text-violet-600', badge: 'bg-violet-100 text-violet-700 border-violet-300' },
  amber: { bg: 'bg-amber-100', text: 'text-amber-600', badge: 'bg-amber-100 text-amber-700 border-amber-300' },
  indigo: { bg: 'bg-indigo-100', text: 'text-indigo-600', badge: 'bg-indigo-100 text-indigo-700 border-indigo-300' },
}

// ─── Main block IDs used for completion check ────────────────────────────────
const MAIN_BLOCK_IDS: BlockId[] = ['block4', 'block5', 'block6', 'block123', 'block22', 'block26']
...
function ErrorReviewSection({
  ...
  // Group incorrect answers by block
...
export default function Lesson29View() {
  const {
    lessonStarted,
    startLesson,
    completedBlocks,
    ...
    return (
      <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 to-slate-100">
            ...
            <Card className="border-0 shadow-xl">
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {LESSON_META.todayWeDont.map((item, i) => (
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center">
                  <p className="text-amber-900 font-medium italic">
                    &ldquo;{LESSON_META.mainPhrase}&rdquo;
                  </p>
                </div>
        ...
        <footer className="py-4 text-center text-xs text-muted-foreground border-t bg-white">
          ЕГЭ Русский: Алгоритмы и практика · {LESSON_META.date}
        </footer>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b shadow-sm">
          ...
          <div className="flex items-center justify-between h-12">
            ...
            <div className="flex items-center gap-1 min-w-0">
              <Button variant="ghost" size="icon" className="shrink-0" asChild>
                <Link href="/lessons" aria-label="На главную">
        ...
        {/* ─── Sidebar / Block list — simple conditional render, no AnimatePresence ── */}
        {sidebarOpen && (
